...
dataDistributionType = "UnBalanced"
SheetName = "balanced"
embeddingName = "glove"
dim = "100d"
## path variables ##
...
#cleandf = Train_And_Test_Comments(Clean, Clean_Test_Data)
#vocab = Vocab(cleandf)
...
def predict():
    if request.method == 'POST':
        ...
        val_dataloader_2 = DataLoader(test_dataset_2, batch_size=1, shuffle=False, num_workers=24)
        result_list_2 = result_dataset_generator(val_dataloader_2)
        result_writer(output_file_path_2, result_list_2)
        df = pd.read_csv(output_file_path_2, delimiter="\t", usecols=['comment', 'predicted_label'])
        las = df["predicted_label"].iloc[-1]
    return render_template('result.html', prediction=las)
