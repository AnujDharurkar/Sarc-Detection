...
CUDA = torch.cuda.is_available()                                                       #torch.cuda.is_available()
...
output_file_path = rootDir +"/" + output + dataDistributionType + "_MHA-BiLSTMoutput_1_head.tsv"
...
class MultiHeadAttBiLSTM(nn.Module):
    ...
    def init_hidden(self):
        ...
        return (Variable(torch.zeros(2 * self.num_layers, self.batch_size, self.hidden_dim)).cuda(),
                Variable(torch.zeros(2 * self.num_layers, self.batch_size, self.hidden_dim)).cuda())         #cuda while training
...
class Trainer:
    def __init__(self, modelName, model, train_partial_dataset, val_dataset, loss_function, optimizer,
        ...
        self.device = torch.device(device if torch.cuda.is_available() else 'cpu')          #torch.cuda.is_available()
        self.model.to(self.device)
        print('using device: ', self.device)

    def _train(self, train_dataset, num_epochs=10, clip=True):
...
def result_dataset_generator(val_dataloader):
    p = 0
    result_dataset_list = []
...
if __name__ == "__main__":
    ...
    val_dataloader = DataLoader(test_dataset, batch_size=1, shuffle=False, num_workers=8)
    ...
    hidden_size = 100
    output_size = 1
    modelName = "Bal_MHA-BiLSTM_1_head"
    ...
    result_writer(output_file_path, result_list)
